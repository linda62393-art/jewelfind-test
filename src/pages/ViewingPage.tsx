...
export function ViewingPage() {
  ...
  return <section className="px-5 pb-10 pt-7"><Link to={`/products/${product.id}`} className="inline-block min-h-11 text-sm text-ink/60">← 返回商品詳情</Link>
    ...
    <div className="my-6 flex items-center gap-4 rounded-2xl bg-white p-3"><img src={product.images[0]} alt={product.name} className="h-20 w-20 rounded-xl object-contain" /><div><p className="font-serif text-lg">{product.name}</p><p className="mt-2 text-xs text-ink/50">可看貨：{product.availableLocations.join('、')}</p></div></div>
